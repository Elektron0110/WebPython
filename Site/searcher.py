...
def fly(рейс: str | list[str]):
    if isinstance(рейс, str):
        рейс = рейс.split(' ')
    компания = рейс[0]
    компания = компания.replace(
        'SU', 'AFL' if not 6000 < int(рейс[1]) < 7000 else 'SDM')
    ...
    for ac in Авиакомпания:
        if рейс == ac.callsign:
            ...
            if самолёт['Аэропорт отправления']:
                отправления = fr_api.get_airport(
                    самолёт['Аэропорт отправления'])
    ...
    return str(самолёт).replace("{", "").replace("}", "").replace("[", "").replace(
