...
def show_graph():
    if os.path.isfile(f'graph/{dt.today().strftime('%Y.%m.%d')}'):
        datae: dict[str, dict[str, str]] = json.loads(
            open(f'graph/{dt.today().strftime('%Y.%m.%d')}', 'r').read())
        # Create a plot
        plt.figure()
        plt.plot([dt.strptime(t, '%Y.%m.%d %H:%M:%S') for t in datae],  # type: ignore
                 [[float(datae[t][k]) for k in datae[t]] for t in datae])
...
def index():
    ...
    dt_0 = (dt.strptime(date, '%Y.%m.%d') - td(1)).strftime(
        '%Y.%m.%d') if dt.strptime(date, '%Y.%m.%d') >= dt(2025, 8, 18) else None
    dt_1 = (dt.strptime(date, '%Y.%m.%d') + td(1)).strftime(
        '%Y.%m.%d') if dt.strptime(date, '%Y.%m.%d') < dt.today() - td(1) else None
    # Пример данных
    if date in os.listdir('graph'):
        datae: dict[str, dict[str, str]] = json.loads(
            open(f'graph/{date}', 'r').read())
...
def press():
    ...
    if not date:
        date = dt.today().strftime("%Y.%m.%d")
    dt_0 = (dt.strptime(date, '%Y.%m.%d') - td(1)).strftime(
        '%Y.%m.%d') if dt.strptime(date, '%Y.%m.%d') > dt(2026, 1, 1) else None
    dt_1 = (dt.strptime(date, '%Y.%m.%d') + td(1)).strftime(
        '%Y.%m.%d') if dt.strptime(date, '%Y.%m.%d') < dt.today() - td(1) else None
    # Пример данных
    if date in os.listdir('graph'):
...
def exindex(days):
    ...
    if days not in [3, 5, 7, 10, 15]:
        return '<script>alert("Хитро")</script><meta http-equiv="refresh" content="0; URL=/train" />'
    weather: dict[str, list[str | dt | float]] = dict(
        topic=[], time=[], value=[])
...
def sun():
    ...
    dt_1 = (dt.strptime(date, '%Y.%m.%d') + td(1)).strftime(
        '%Y.%m.%d') if dt.strptime(date, '%Y.%m.%d') < dt.today() - td(1) else None
    # Пример данных
    open(f'sun/{date}', 'w').write(
        get(f'https://xras.ru/txt/xray_S0KJ_{date.replace('.', '')}.json').text)
    datae: list[dict[str, str]] = json.loads(
        open(f'sun/{date}', 'r').read())['data']
    ...
    fig.update_yaxes(range=[-9 * 0.000001, -2 * 0.000001],
                     tickvals=[-9 * 0.000001, -8 * 0.000001, -7 * 0.000001, -6 *
                               0.000001, -5 * 0.000001, -4 * 0.000001, -3 * 0.000001, -2 * 0.000001],
...
def sunaver():
    ...
    datae: list[dict[str, str]] = json.loads(
        open(f'sun/{date}', 'r').read())['data']
    weather: dict[str, list[dt | float | str]] = dict(
        time=[], value=[], type=[])
    sun: list[int] = []
    for tv in datae:
...
def locate():
    ...
    dm = int(res['sunset'][res['sunset'].find(
        ':') + 1:res['sunset'].rfind(':')])
