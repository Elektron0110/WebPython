...
def alice():
    ...
    if req["session"]["new"] and not req['request']['command']:
        ...
        text = txt.split(' ')
        if len(text) == 2:
            response['response']['text'] = fly(text).replace(
                '<br>', '\n')  # str(text) #'(Ответ)' #
            if fly(text):
                response['response']['end_session'] = True
            else:
                open('errors', 'a').write(
                    txt + ' ' + datetime.now().strftime("%H:%M") + ' ' + req['meta']['client_id'] + '\n')
            ...
            json.dump({'response': response, 'request': req},
                      open('alice.json', 'w'))
        else:
            response['response']['text'] = 'Прошу прощения, но я не могу найти информацию про рейс, номер которого состоит только из букв. В его конце ' \
                'должны быть четыре какие-либо цифры.\nВозможно Ваше устройство не расслышало Вас. Попробуйте ещё раз.'
    else:
        response['response']['text'] = 'Прошу прощения, но я не могу найти информацию про рейс, номер которого состоит только из цифр. В его начале ' \
